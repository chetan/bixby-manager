...
config_path = "/var/www/bixby/current/config/deploy/thin.yml"
...
(0...num_servers).each do |i|
  ...
  God.watch do |w|
    ...
    w.start = "thin start -C #{config_path} -o #{number}"
    w.start_grace = 10.seconds

    w.stop = "thin stop -C #{config_path} -o #{number}"
    w.stop_grace = 10.seconds

    w.restart = "thin restart -C #{config_path} -o #{number}"
    ...
    w.start_if do |c|
      c.condition(:process_exits)
    end

    # restart if using too much memory or cpu
